...
void testApp::keyPressed(int key) {
	...
	if(isalnum((unsigned char) key)) {
		...
		velocity = 64;
		midiOut.sendNoteOn(synthChan, note,  velocity);
        midiOut.sendNoteOn(effectsChan, note,  velocity);
...
void testApp::mouseDragged(int x, int y, int button) {
	...
	xPad = ofMap(x, 0, ofGetWidth(), 0, 127);
	midiOut.sendControlChange(synthChan, 12, xPad);
    midiOut.sendControlChange(effectsChan, 12, xPad);

	
	// y pos controls the pitch bend
	yPad = ofMap(y, 0, ofGetHeight(), 127, 0);
	midiOut.sendControlChange(synthChan, 13, yPad);
    midiOut.sendControlChange(effectsChan, 13, yPad);
